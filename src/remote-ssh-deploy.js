...
async function deploy({ profile, runtime, deps = {} }) {
  ...
  if (typeof profile.hostPrefix === "string" && profile.hostPrefix.length > 0) {
    ...
    const args = buildSshArgs(profile).concat([
      "cat > ~/.claude/hooks/deskbuddy-host-prefix",
    ]);
...
//
// `~/.deskbuddy-codex-monitor.pid` on the remote is a fixed marker holding the
...
async function startCodexMonitor({ profile, runtime = null, deps = {} }) {
  ...
  const cleanCmd =
    "[ -f ~/.deskbuddy-codex-monitor.pid ] && kill $(cat ~/.deskbuddy-codex-monitor.pid) 2>/dev/null; " +
    "rm -f ~/.deskbuddy-codex-monitor.pid; true";
  ...
  const startCmd =
    ...
    "> /dev/null 2>&1 & echo $! > ~/.deskbuddy-codex-monitor.pid";
...
async function stopCodexMonitor({ profile, runtime = null, deps = {} }) {
  ...
  const cmd =
    "[ -f ~/.deskbuddy-codex-monitor.pid ] && kill $(cat ~/.deskbuddy-codex-monitor.pid) 2>/dev/null; " +
    "rm -f ~/.deskbuddy-codex-monitor.pid";
