...
//
// followPet=false → bottom-right of the work area (default DeskBuddy behavior).
//
...
function showPermissionBubble(permEntry) {
  ...
  // macOS: defer full visibility restoration to avoid activating DeskBuddy
...
// Telegram-only "Tool input hidden by DeskBuddy." card would let the user approve
// a black box.
...
function buildRemoteApprovalPayload(permEntry) {
  ...
  // Label is "Folder" (not "Session") on purpose: the pinned deskbuddy-connect
...
function cleanup() {
  ...
  for (const perm of [...pendingPermissions]) {
    ...
    if (perm.isCodex || perm.isQwenCode || perm.isCopilotCli || perm.isAntigravity || perm.isHermes) resolvePermissionEntry(perm, "no-decision", "DeskBuddy is quitting");
    else resolvePermissionEntry(perm, "deny", "DeskBuddy is quitting");
  }
}
