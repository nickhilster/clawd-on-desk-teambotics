...
shipped DeskBuddy binary — they live here as ops aids for re-validating wire
...
| `e2e-hook.js` | Drives the production `hooks/copilot-hook.js` end-to-end against a mock DeskBuddy HTTP server. Verifies allow / deny / 204 / 500 / connection-refused / malformed-stdin / wrong-server-header all end in `exit 0` with the right stdout. Run before merging any change to `copilot-hook.js` or `copilot-install.js`. |

Both scripts are safe to run by hand. Neither is invoked by DeskBuddy itself
...
2. Edit `~/.copilot/hooks/hooks.json` and **replace** the DeskBuddy
   ...
   Replacing (not appending) is on purpose: DeskBuddy's own permission hook
   would otherwise compete with capture for the same event. After the
   capture run, restart DeskBuddy to have it re-register its own entry.
...
4. Read the resulting JSON lines from the configured log path (default
   `%APPDATA%\deskbuddy\debug.log` on Windows, or wherever
   `DESKBUDDY_COPILOT_HOOK_DEBUG_PATH` points). Scrub any tokens / private
...
Stopping DeskBuddy is a prerequisite (mock server binds to `127.0.0.1:23333`,
the same port DeskBuddy's own state server uses). The harness prints a
human-readable error if it can't take the port and bails immediately.

It runs 9 scenarios sequentially:

1. DeskBuddy 200 allow → hook stdout `{"behavior":"allow"}`
2. DeskBuddy 200 deny + message → hook stdout `{"behavior":"deny","message":"…"}`
3. DeskBuddy 204 no-decision → hook empty stdout
4. DeskBuddy 500 internal error → hook empty stdout
...
9. Server identity header missing → hook empty stdout (non-DeskBuddy response)
